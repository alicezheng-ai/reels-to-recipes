...
Turns saved Instagram recipe videos into a searchable bilingual recipe library, using Google Drive as the backend and Gemini for extraction. Runs entirely inside your own Google account — one Apps Script project, one Drive folder, one API call out.

You drop a video into a Drive folder. Fifteen minutes later it's a recipe card with tags, ingredients, steps, and a note about whether it can be adapted for a baby, in English and Chinese.

## How it works

```
phone → Drive/inbox → processInbox() ──upload+prompt──> Gemini
                            │                              │
                            │         <──JSON+flags────────┘
...
A time-driven trigger runs `processInbox()` every fifteen minutes; the web app's **Import** button runs the same function on demand. Extraction is one Gemini call against the video itself — speech and on-screen text together — returning a structured record against a closed tag vocabulary.
...
| `Index.html` | The interface. Served by `HtmlService`; the recipe library is injected at render time, so there's no second round trip. |
...
5. Run `setup()` once. It creates the Drive folders, installs the trigger, and logs the inbox link.
...
|---|---|
| `dryRun(fileName?)` | Extracts one video, logs the full record, and keeps the video regardless of `KEEP_VIDEOS`. The way to test prompt changes. |
| `whatsInInbox()` | Lists what the script sees in the inbox — name, mime type, size, and whether it will be picked up. |
| `showFailures()` | Prints the reason file for everything in `/failed`. |
